...
.addComponents(
    ...
    .setCustomId('ertucum')
    .setPlaceholder(`Başka Sıralamayı Gör`)
    .addOptions([
    { label: 'PP', description: 'PP Sıralaması', value: 'pp'},
    { label: 'Gif', description: 'Gif Sıralaması', value: 'gif'},
    ]),
    );
    ...
    .setDescription(map||`Kimse hiçbir şey paylaşmamış`)
    ...
    collector.on("collect", async (interaction) => {
      ...
      if (interaction.values[0] === "pp") {
        ...
        let embed = new EmbedBuilder()
        .setThumbnail(message.guild.iconURL({ dynamic: true, size: 2048 }))
        .setDescription(map||`Kimse fotoğraf paylaşmamış`)
      ...
      let embed = new EmbedBuilder()
    .setThumbnail(message.guild.iconURL({ dynamic: true, size: 2048 }))
    .setDescription(map||`Kimse GIF paylaşmamış`)
    msg.edit({embeds: [embed], components: [row]}) 
