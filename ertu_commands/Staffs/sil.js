...
execute:async (client, message, args) => {
    ...
    if (args[0] && args[0] < 99 && args[0] > 0 && !isNaN(args[0])) {
        ...
        message.channel.send({ content: `${args[0]} bu odadaki mesajlar silindi <#${message.channel.id}>` }).then((e) => setTimeout(() => { e.delete(); }, 5000));    
    } else {
      ...
      collector.on("collect", async (button) => {
        ...
        if (button.customId === "ten") {
          ...
          message.channel.send({ embeds: [new EmbedBuilder().setDescription(`10 mesaj silindi.`)]}).then((e) => setTimeout(() => { e.delete(); }, 5000));
        }
        if (button.customId === "twentyfive") {
          await message.delete();
          await message.channel.bulkDelete(25);
          message.channel.send({ embeds: [new EmbedBuilder().setDescription(`25 mesaj silindi.`)]}).then((e) => setTimeout(() => { e.delete(); }, 5000));
        }
        if (button.customId === "fifty") {
          await message.delete();
          await message.channel.bulkDelete(50);
          message.channel.send({ embeds: [new EmbedBuilder().setDescription(`50 mesaj silindi.`)]}).then((e) => setTimeout(() => { e.delete(); }, 5000));

        }
        if (button.customId === "onehundred") {
          await message.delete();
          await message.channel.bulkDelete(99);
          message.channel.send({ embeds: [new EmbedBuilder().setDescription(`100 mesaj silindi.`)]}).then((e) => setTimeout(() => { e.delete(); }, 5000));
        }
